...
public class MainMenuUI_TMP : MonoBehaviour
{
    [Header("메인 메뉴 UI")]
    ...
    public TextMeshProUGUI[] saveSlotTexts = new TextMeshProUGUI[3];
    public TextMeshProUGUI[] saveSlotInfoTexts = new TextMeshProUGUI[3];
    ...
    public TextMeshProUGUI[] characterNameTexts = new TextMeshProUGUI[4];
    ...
    private void UpdateSaveSlotUI(SaveData[] saveSlots)
    {
        ...
        for (int i = 0; i < saveSlotButtons.Length && i < saveSlots.Length; i++)
        {
            ...
            if (slotData.isEmpty)
            {
                ...
                saveSlotInfoTexts[i].text = "New Game";
            }
            else
            {
                saveSlotTexts[i].text = slotData.slotName;
                saveSlotInfoTexts[i].text = $"Level {slotData.level} | Day {slotData.dayCount}\n{slotData.lastSaveTime:yyyy/MM/dd HH:mm}";
            }
        }
    }
